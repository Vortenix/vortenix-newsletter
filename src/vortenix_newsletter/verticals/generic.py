...
class GenericVertical:
    ...
    async def analyse(self,documents: list[SourceDocument])->VerticalResearchResult:
        ...
        for doc in documents:
            ...
            findings.append(ResearchFinding(vertical_id=self.vertical_id,title=doc.title,summary=summary,development=f"Observed: {summary}",significance="Interpretation: this development matches configured research priorities.",pain_points=pain,affected_companies=companies,opportunities=["Monitor adoption and solution activity."],risks=["Evidence is limited to the cited source."],relevance_score=relevance,novelty_score=.6,significance_score=.6,confidence_score=.7,evidence_score=.7,recency_score=.9,citations=[Citation(source_document_id=doc.id,source_title=doc.title,url=doc.url,supporting_excerpt=summary[:300])]))
